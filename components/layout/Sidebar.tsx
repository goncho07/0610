...
const Sidebar: React.FC = () => {
  ...
  const linkClasses = "flex items-center px-4 py-4 text-slate-600 dark:text-slate-100 rounded-lg hover:bg-indigo-100 dark:hover:bg-slate-800 hover:text-indigo-700 dark:hover:text-indigo-400 transition-all duration-150 group relative font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-indigo-500 dark:focus-visible:ring-offset-slate-900";
        ...
        <ul className="space-y-1">
          {directorNavItems.map((item) => (
            <li key={item.to} onMouseEnter={() => setHoveredItem(item.to)} onMouseLeave={() => setHoveredItem(null)}>
              <NavLink 
                to={item.to} 
                end={item.to === '/' || item.to === '/asistencia'}
                className={({ isActive }) => `${linkClasses} ${isActive ? activeLinkClasses : ''}`}
              >
                <item.icon className={`h-8 w-8 shrink-0 transition-all ${isSidebarCollapsed ? 'mx-auto' : 'mr-4'}`} />
                <AnimatePresence>
                {!isSidebarCollapsed && (
                  <motion.span
